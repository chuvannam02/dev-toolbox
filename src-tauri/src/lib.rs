...
async fn trigger_jenkins_job(
    ...
    let trigger_url = if params.is_empty() {
        ...
        format!("{}/job/{}/buildWithParameters", url.trim_end_matches('/'), job_name)
    };
    ...
    if !res.status().is_success() {
        return Err(format!("❌ Lỗi Trigger (HTTP {}): Kiểm tra lại Token hoặc Tên Job.", res.status()));
    }

    let _ = app.emit("jenkins-log", format!("[Dev Toolbox] 🚀 Đã gửi lệnh trigger Job: {}. Đang đợi Jenkins xử lý...\n", job_name));
    ...
    tokio::spawn(async move {
        ...
        loop {
            let log_url = format!(
                ...
                url.trim_end_matches('/'), job_name, start_byte
            );
            
            if let Ok(response) = client.get(&log_url).basic_auth(&user, Some(&token)).send().await {
                // Jenkins trả về Header 'X-More-Data' nếu Job vẫn đang chạy
                let more_data = response.headers().get("X-More-Data").is_some();
                let new_size = response.headers().get("X-Text-Size")
...
fn analyze_build_log(log_text: String) -> LogAnalysis {
    ...
    for line in lines {
        ...
        if lower.contains("finished: success") { status = "SUCCESS".to_string(); }
        if lower.contains("finished: failure") || lower.contains("build failure") { status = "FAILED".to_string(); }
...
fn get_docker_containers(ssh: SshConfig) -> Result<Vec<DockerContainer>, String> {
    ...
    cmd.args(["ps", "-a", "--format", "{{.ID}}|{{.Names}}|{{.Status}}|{{.Image}}"]);

    let output = cmd.output().map_err(|e| format!("Lỗi thực thi lệnh: {}", e))?;
...
#[tauri::command]
fn get_docker_logs(container_id: String, tail: String, grep: String, ssh: SshConfig) -> Result<String, String> {
    let mut cmd = Command::new(if ssh.enabled { "ssh" } else { "docker" });
...
fn get_launcher_apps(state: tauri::State<AppState>) -> Result<Vec<AppItem>, String> {
    ...
    let mut stmt = conn.prepare("SELECT id, name, path, icon FROM launcher_apps").unwrap();
    let iter = stmt.query_map([], |row| {
        Ok(AppItem { id: Some(row.get(0)?), name: row.get(1)?, path: row.get(2)?, icon: row.get(3)? })
    }).unwrap();
...
fn quote_for_windows_cmd(argument: &str) -> String {
    ...
    if argument.chars().any(|ch| ch.is_whitespace() || "\"&|<>()^".contains(ch)) {
...
#[tauri::command]
fn save_credential(state: tauri::State<AppState>, name: String, username: String, password: String) -> Result<(), String> {
    let conn = state.db.lock().unwrap();
    conn.execute("INSERT INTO credentials (name, username) VALUES (?1, ?2)", params![&name, &username])
        .map_err(|e| e.to_string())?;
...
fn get_credentials(state: tauri::State<AppState>) -> Result<Vec<Credential>, String> {
    let conn = state.db.lock().unwrap();
    let mut stmt = conn.prepare("SELECT id, name, username FROM credentials").map_err(|e| e.to_string())?;
    let creds_iter = stmt.query_map([], |row| {
        Ok(Credential { id: row.get(0)?, name: row.get(1)?, username: row.get(2)? })
    }).map_err(|e| e.to_string())?;

    let mut result = Vec::new();
    for cred in creds_iter { if let Ok(c) = cred { result.push(c); } }
    Ok(result)
}
...
fn detect_and_format(raw_text: String) -> FormatResult {
    ...
    if text_trim.is_empty() {
        return FormatResult { format_type: "unknown".into(), is_valid: false, formatted_text: "".into(), error_msg: None };
    }

    // 0. Convert common Bash/Postman cURL exports into a command that CMD can paste and run.
    if matches!(text_trim.split_whitespace().next().map(|part| part.to_ascii_lowercase()).as_deref(), Some("curl") | Some("curl.exe")) {
    ...
    if upper.starts_with("SELECT") || upper.starts_with("INSERT") || upper.starts_with("UPDATE") || upper.starts_with("DELETE") || upper.starts_with("CREATE") {
        let formatted = sqlformat::format(text_trim, &sqlformat::QueryParams::None, &sqlformat::FormatOptions::default());
        return FormatResult {
    ...
    FormatResult {
        ...
        is_valid: true,
        formatted_text: raw_text,
        error_msg: Some("Không nhận dạng được định dạng đặc biệt, hiển thị như văn bản thường.".into()),
    }
}
...
fn check_grammar(text: String) -> Result<Vec<String>, String> {
    ...
    if text.contains(" ,") || text.contains(" .") {
        suggestions.push("⚠️ Dấu câu: Không được có khoảng trắng trước dấu phẩy hoặc dấu chấm.".into());
    }

    // 2. Kiểm tra lỗi sai kinh điển của Dev khi viết Email tiếng Anh
    if text_lower.contains("please to inform") {
        suggestions.push("💡 Ngữ pháp: Hãy đổi 'please to inform' thành 'pleased to inform'.".into());
    }
    if text_lower.contains("looking forward to hear") {
        suggestions.push("💡 Ngữ pháp: Cấu trúc đúng là 'looking forward to hearing' (V-ing).".into());
    }
    ...
    if text_lower.contains("advices") || text_lower.contains("informations") {
        suggestions.push("💡 Từ vựng: 'Advice' và 'Information' là danh từ không đếm được, không có 's'.".into());
    }
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            let app_data_dir = app.path().app_data_dir().expect("Không lấy được thư mục AppData");
        ...
        .invoke_handler(tauri::generate_handler![save_credential, get_credentials, detect_and_format, json_to_excel,csv_to_excel,get_launcher_apps,
            add_launcher_app,
            launch_items,
            execute_ansible, get_docker_containers, get_docker_logs,
            check_grammar,
            analyze_build_log,
            trigger_jenkins_job,
...
fn csv_to_excel(csv_text: String) -> Result<String, String> {
    ...
    for (col, header) in headers.iter().enumerate() {
        worksheet.write_string(0, col as u16, header).map_err(|e| e.to_string())?;
    }
    ...
    for (row, result) in rdr.records().enumerate() {
        ...
        for (col, field) in record.iter().enumerate() {
            worksheet.write_string((row + 1) as u32, col as u16, field).map_err(|e| e.to_string())?;
        }
    }

    // Lưu file
    let desktop_dir = dirs::desktop_dir().unwrap_or_else(|| std::env::temp_dir());
    let file_path = desktop_dir.join(format!("CSV_Export_{}.xlsx", chrono::Local::now().format("%Y%m%d_%H%M%S")));
    
    workbook.save(&file_path).map_err(|e| format!("Lỗi lưu file: {}", e))?;
...
fn json_to_excel(json_text: String, array_path: String) -> Result<String, String> {
    // 1. Parse JSON
    let parsed: Value = serde_json::from_str(&json_text).map_err(|e| format!("JSON không hợp lệ: {}", e))?;
    ...
    // 3. Ép kiểu về Array
    let arr = current_node.as_array()
        .ok_or_else(|| "Dữ liệu tại path được chỉ định không phải là một mảng (Array) []".to_string())?;
        ...
        if let Some(obj) = item.as_object() {
            for (col, header) in headers.iter().enumerate() {
                if let Some(val) = obj.get(header) {
                    ...
                    worksheet.write_string((row + 1) as u32, col as u16, &val_str).map_err(|e| e.to_string())?;
                }
            }
        }
    }
    ...
    let desktop_dir = dirs::desktop_dir().unwrap_or_else(|| std::env::temp_dir());
    let file_path = desktop_dir.join(format!("Export_{}.xlsx", chrono::Local::now().format("%Y%m%d_%H%M%S")));
