...
import { generateFakeDataFromSchema } from "../services/databaseService";
import ManualColumnsSection, {
	ColumnRow,
	newColumn,
} from "./ManualColumnsSection";

type GeneratorMode = "manual" | "database";
...
export default function FakeDataGenerator() {
	...
	const [connection, setConnection] = useState<DatabaseConfig>({
		dbType: "PostgreSQL",
	...
	const [databaseSchema, setDatabaseSchema] = useState<TableSchema | null>(
		null,
	);
	...
	async function handleGenerate() {
		...
		if (mode === "database") {
			if (!databaseSchema) {
				setError("Vui lòng kết nối database và chọn bảng.");
			...
			try {
				const rows = await generateFakeDataFromSchema(
					databaseSchema,
					recordCount,
				);
				...
				setError(
					typeof e === "string"
						? e
						: "Sinh dữ liệu từ schema thất bại.",
				);
			} finally {
				setLoading(false);
			}
	...
	// 		<div style={styles.section}>
	// 			<div style={styles.label}>Nguồn schema</div>
		...
		<div className="fdg" style={styles.root}>
				...
				<>
					...
					<div style={styles.section}>
						...
						<select
							value={dbType}
							onChange={(e) =>
								handleDbChange(e.target.value as DbType)
							}
							style={styles.select}
						>
							{DB_TYPES.map((db) => (
			...
			{mode === "database" && (
				<div style={styles.section}>
					<DatabaseConnectionPanel
						...
						onSchemaLoaded={setDatabaseSchema}
					/>
				</div>
			)}
			...
			>
				...
				<button
					...
					onClick={handleGenerate}
					disabled={
						loading || (mode === "database" && !databaseSchema)
					}
					style={styles.primaryButton}
				>
					{loading ? "Đang sinh dữ liệu…" : "Generate"}
			...
			{results && results.length > 0 && (
				<div style={styles.section}>
					...
					>
						...
						>
							✓ Đã sinh {results.length} bản ghi
						</span>

						{mode === "database" && databaseSchema && (
							<span
								style={{
									color: "var(--fdg-muted)",
									fontSize: 12,
							...
							>
								{databaseSchema.schema}.{databaseSchema.table}
							</span>
						)}
					...
					<div style={styles.tableWrap}>
						...
						>
							...
							<tbody>
								{results.slice(0, 200).map((row, index) => (
									<tr
										key={index}
										style={
											index % 2 === 1
						...
						{results.length > 200 && (
							...
							>
								Chỉ hiển thị 200/
								{results.length} dòng đầu tiên. Export JSON/CSV
			...
			{results && results.length === 0 && !loading && (
				<div style={styles.section}>
					...
					>
						Generator không trả về bản ghi nào.
					</div>
				</div>
			)}
		</div>
